...
if [ "$TRAIN_STAGE" = "1" ]; then
    ...
    LORA_ENABLE=False
    AVGEN_PROJ_PATH=""
    DATASET_PATH="${DATA_ROOT}/train/MM-PreTrain/${DATASET_NAME}.json"
    RUN_NAME="javisgpt_stage1_mm_pretrain_avgen_align"
elif [ "$TRAIN_STAGE" = "2" ]; then
    ########### stage2 - jav-ft ############
    VIDEO_FOLDER="${DATA_ROOT}/train/AV-FineTune/video"
    AUDIO_FOLDER="${DATA_ROOT}/train/AV-FineTune/audio"
    DATASET_NAME="stage2_av_ft_gen"
    ...
    DATASET_PATH="${DATA_ROOT}/train/AV-FineTune/${DATASET_NAME}.json"
    RUN_NAME="javisgpt_stage2_av_finetune_avgen_align"
else
...
deepspeed --include="localhost:${GPUS}" --master_port 29603 \
    ...
    --training_stage "audio_video_gen_align" \
    --force_sample True \
    --bf16 True \
    --fp16 False \
    --run_name $RUN_NAME \
    ...
    --dataloader_drop_last True >${LOG_DIR}/output.txt 2>${LOG_DIR}/error.txt &
